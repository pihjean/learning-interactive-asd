...
const MediumGame = () => {
  ...
  const API_URL = 'http://daetsnedlearning.site/backend/game_medium.php';

  useEffect(() => {
    console.log('Received in MediumGame:', { category, difficulty });
  ...
  const validateForm = () => {
    if (!formData.answer1.trim() || !formData.answer2.trim()) {
      setErrorMessage('Please enter both required answers');
      return false;
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (modalType === 'edit' && formData.medium_id) {
        saveFormData.append('medium_id', formData.medium_id);
      }
      ...
      if (modalType === 'edit') {
        const pathFields = {
          ...
          'wrong_answer2': 'wrong_answer2_path', 'final_achievement': 'final_achievement_path'
        };
      ...
      if (result.success) {
        const successMsg = modalType === 'add' ? 
          ...
          `Game ID ${formData.medium_id} updated successfully!`;
  ...
  const handleDelete = async () => {
    try {
      ...
      console.log('Deleting game:', {
        medium_id: selectedGame.medium_id,
        category: category,
      ...
      if (result.success) {
        const successMsg = `Game ID ${selectedGame.medium_id} deleted successfully!`;
  ...
  const renderFormContent = () => {
    return (
      <div className="games-form-layout">
        <div className="games-form-full-width">
          ...
          <div className="games-list-container">
            {games.length === 0 ? (
              ...
              <table className="games-table">
                ...
                <tbody>
                  {games.map((game) => (
                    <tr key={game.medium_id}>
                      <td>{game.medium_id}</td>
