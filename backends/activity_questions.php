...
try {
    ...
    if ($method === 'POST') {
        ...
        // Handle action
        switch ($data['action']) {
            case 'save_activity_questions':
                saveActivityQuestions($conn, $data);
                break;
            default:
    ...
    elseif ($method === 'GET') {
        ...
        switch ($_GET['action']) {
            ...
            case 'fetch_questions':
                ...
                break;
            case 'fetch_activity_questions':
                if (!isset($_GET['category']) || !isset($_GET['difficulty'])) {
                    throw new Exception("Missing category or difficulty parameter");
                }
                fetchActivityQuestions($conn, $_GET['category'], $_GET['difficulty']);
                break;
            default:
...
function saveActivityQuestions($conn, $data) {
    // Validate required parameters
    if (!isset($data['category']) || !isset($data['difficulty'])) {
    ...
    // Prepare SQL query
    $sql = "INSERT INTO activity_questions (category, difficulty, question1, question2, question3) 
    ...
    if ($conn->query($sql) === TRUE) {
        echo json_encode(["success" => true, "message" => "Activity questions assigned successfully"]);
    } else {
        throw new Exception("Error assigning activity questions: " . $conn->error);
    }
}

// Function to fetch all assigned categories and difficulties
function fetchAssignedCategories($conn) {
    $sql = "SELECT category, difficulty FROM activity_questions GROUP BY category, difficulty";
...
// Function to fetch assigned questions for a specific category and difficulty
function fetchActivityQuestions($conn, $category, $difficulty) {
    $category = $conn->real_escape_string($category);
    $difficulty = $conn->real_escape_string($difficulty);
    
    $sql = "SELECT * FROM activity_questions WHERE category = '$category' AND difficulty = '$difficulty'";
    $result = $conn->query($sql);
    
    if (!$result) {
        throw new Exception("Error fetching activity questions: " . $conn->error);
    }
