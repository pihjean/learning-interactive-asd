...
function debugLog($message) {
    ...
    $debugFile = $_SERVER['DOCUMENT_ROOT'] . '/debug_game_medium.log';
...
function getGames($conn, $params) {
    ...
    while ($row = $result->fetch_assoc()) {
        $game = [
            'medium_id' => intval($row['medium_id']),
            'answer1' => $row['answer1'],
...
function addGame($conn, $upload_dir) {
    ...
    try {
        ...
        if (!isset($_POST['answer1'], $_POST['answer2'], $_POST['category'])) {
            throw new Exception("Missing required fields: answer1, answer2, or category");
        }
        ...
        if (empty($answer1) || empty($answer2) || empty($category)) {
        ...
        $max_id_stmt = $conn->prepare("SELECT MAX(medium_id) FROM game_medium");
        if (!$max_id_stmt) {
        ...
        $medium_id = $max_id ? $max_id + 1 : 1;
        debugLog("Generated new medium_id: " . $medium_id);
        
        $final_dir = $upload_dir . $category . '/' . $difficulty . '/' . $medium_id . '/';
        ...
        $fileFields = ['map', 'lesson', 'question1', 'achievement1', 'wrong_answer1', 
                      'question2', 'achievement2', 'wrong_answer2', 'final_achievement'];
        ...
        foreach ($fileFields as $field) {
            if (isset($_POST[$field . '_temp_path']) && !empty($_POST[$field . '_temp_path'])) {
                ...
                if (!file_exists($temp_path)) {
                    debugLog("Warning: Temp file not found: $temp_path");
                ...
                if (!rename($temp_path, $final_file_path)) {
                    throw new Exception("Failed to move file from temp to final location for $field");
                }
                
                $relative_path = 'uploads/' . $category . '/' . $difficulty . '/' . $medium_id . '/' . $filename;
        ...
        debugLog("Preparing database insert for medium_id: $medium_id");
        
        $stmt = $conn->prepare("
            INSERT INTO game_medium 
            (medium_id, category, answer1, q1_wrong_answer1, q1_wrong_answer2, 
             answer2, q2_wrong_answer1, q2_wrong_answer2, status,
             map_path, lesson_path, question1_path, achievement1_path, wrong_answer1_path,
             question2_path, achievement2_path, wrong_answer2_path, final_achievement_path) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'not in use', ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        ...
        debugLog("Game added successfully with ID: $medium_id");
...
function updateGame($conn, $upload_dir) {
    ...
    try {
        ...
        if (!isset($_POST['medium_id'])) {
            throw new Exception("Missing medium_id parameter");
        }
        ...
        $check_stmt = $conn->prepare("SELECT COUNT(*) FROM game_medium WHERE medium_id = ?");
        if (!$check_stmt) {
            throw new Exception("Failed to prepare check statement: " . $conn->error);
        }
        
        $check_stmt->bind_param("i", $medium_id);
        $check_stmt->execute();
        ...
        if ($count == 0) {
            throw new Exception("Game with ID $medium_id does not exist.");
        }
        ...
        $sql = "UPDATE game_medium SET answer1 = ?, q1_wrong_answer1 = ?, q1_wrong_answer2 = ?, answer2 = ?, q2_wrong_answer1 = ?, q2_wrong_answer2 = ?, category = ?";
        $params = [$answer1, $q1_wrong_answer1, $q1_wrong_answer2, $answer2, $q2_wrong_answer1, $q2_wrong_answer2, $category];
        $types = "sssssss";
        ...
        foreach ($fields as $file_key => $db_field) {
            if (isset($_POST[$file_key . '_temp_path']) && !empty($_POST[$file_key . '_temp_path'])) {
                deleteOldFile($conn, $medium_id, $db_field);
        ...
        $conn->commit();
        debugLog("Game updated successfully with ID: $medium_id, Files updated: $filesUpdated");
...
function deleteOldFile($conn, $medium_id, $db_field) {
    $stmt = $conn->prepare("SELECT $db_field FROM game_medium WHERE medium_id = ?");
    if (!$stmt) {
...
function deleteGame($conn, $upload_dir, $data) {
    ...
    try {
        ...
        $medium_id = intval($data['medium_id']);
        ...
        if ($result->num_rows == 0) {
            throw new Exception("No game found with ID $medium_id");
        }
        ...
        $stmt = $conn->prepare("DELETE FROM game_medium WHERE medium_id = ?");
        if (!$stmt) {
            throw new Exception("Prepare statement failed: " . $conn->error);
        }
        ...
        if (!empty($category)) {
            $game_dir = $upload_dir . $category . '/' . $difficulty . '/' . $medium_id . '/';
