...
function fetchGames($conn, $params) {
    ...
    switch ($difficulty) {
        case 'easy':
            $table = 'game_easy';
            $id_field = 'easy_id';
            $fields = 'easy_id as id, correct_answer, status, introduction_path, map_path, lesson_path, question_path, achievement_path, wrong_answer_path';
            break;
        case 'medium':
            $table = 'game_medium';
            $id_field = 'medium_id';
            $fields = 'medium_id as id, answer1, answer2, status, map_path, lesson_path, question1_path, achievement1_path, wrong_answer1_path, question2_path, achievement2_path, wrong_answer2_path, final_achievement_path';
            break;
        case 'hard':
            $table = 'game_hard';
            $id_field = 'hard_id';
            $fields = 'hard_id as id, answer1, answer2, answer3, status, map_path, lesson_path, question1_path, achievement1_path, wrong_answer1_path, question2_path, achievement2_path, wrong_answer2_path, question3_path, achievement3_path, wrong_answer3_path, final_achievement_path, last_map_path, outro_path';
            break;
        default:
            throw new Exception("Invalid difficulty level");
    }
    
    $sql = "SELECT $fields FROM $table WHERE category = ? ORDER BY $id_field ASC";
    ...
    while ($row = $result->fetch_assoc()) {
        // Process video paths for each game
        $game = ['id' => $row['id'], 'status' => $row['status']];
        
        // Add answers based on difficulty
        if ($difficulty === 'easy') {
            $game['correct_answer'] = $row['correct_answer'];
        } elseif ($difficulty === 'medium') {
            $game['answer1'] = $row['answer1'];
            $game['answer2'] = $row['answer2'];
        } else { // hard
            $game['answer1'] = $row['answer1'];
            $game['answer2'] = $row['answer2'];
            $game['answer3'] = $row['answer3'];
        }
        
        // Add video fields based on what's available for each difficulty
        $videoFields = [];
        switch ($difficulty) {
            case 'easy':
                $videoFields = ['introduction', 'map', 'lesson', 'question', 'achievement', 'wrong_answer'];
                break;
            case 'medium':
                $videoFields = ['map', 'lesson', 'question1', 'achievement1', 'wrong_answer1', 'question2', 'achievement2', 'wrong_answer2', 'final_achievement'];
                break;
            case 'hard':
                $videoFields = ['map', 'lesson', 'question1', 'achievement1', 'wrong_answer1', 'question2', 'achievement2', 'wrong_answer2', 'question3', 'achievement3', 'wrong_answer3', 'final_achievement', 'last_map', 'outro'];
                break;
        }
        ...
        foreach ($videoFields as $field) {
            ...
            if (!empty($row[$pathField])) {
                $game[$field] = ['path' => $row[$pathField]];
            }
        }
...
function fetchAllAssignments($conn) {
    debugLog("fetchAllAssignments called");
    
    $stmt = $conn->prepare("SELECT day, category FROM games_per_day");
    if (!$stmt) {
        throw new Exception("Prepare statement failed: " . $conn->error);
    }
    ...
    while ($row = $result->fetch_assoc()) {
        debugLog("Found assignment: " . json_encode($row));
        $assignments[] = $row;
    }
    
    debugLog("Total assignments found: " . count($assignments));
...
function saveDayAssignment($conn, $data) {
    ...
    debugLog("Saving assignment: Day=$day, Category=$category");
    
    // Validate that all difficulty levels have at least one game "in use"
    $difficulties = ['easy', 'medium', 'hard'];
    $tables = ['game_easy', 'game_medium', 'game_hard'];
    $missingDifficulties = [];
    ...
    for ($i = 0; $i < count($difficulties); $i++) {
        ...
        debugLog("$difficulty games in use for $category: $count");
        
        if ($count == 0) {
            $missingDifficulties[] = strtoupper($difficulty);
        }
    }
    
    if (count($missingDifficulties) > 0) {
        $errorMessage = "Cannot save assignment. Please set at least one game as 'IN USE' for ALL difficulty levels. Missing: " . implode(', ', $missingDifficulties);
        debugLog("Validation failed: " . $errorMessage);
        throw new Exception($errorMessage);
    }
    
    debugLog("All difficulty levels validation passed");
    ...
    try {
        ...
        debugLog("Found IDs: Easy=$easy_id, Medium=$medium_id, Hard=$hard_id");
        ...
        if ($exists > 0) {
            ...
            $update_stmt->close();
            debugLog("Updated existing assignment");
        } else {
            ...
            $insert_stmt->close();
            debugLog("Inserted new assignment");
        }
        
        $conn->commit();
        debugLog("Assignment saved successfully");
        ...
        $conn->rollback();
        debugLog("Error saving assignment: " . $e->getMessage());
        throw $e;
    }
}
?>