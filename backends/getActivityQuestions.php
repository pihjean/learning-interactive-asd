...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        // Check if studentID and progID are provided
        ...
        $progID = $conn->real_escape_string($_GET['progID']);
        
        // Log the request parameters
        error_log("Fetching activity questions for studentID: $studentID, progID: $progID");
        
        // Step 1: Get questions from activity_progress
        $sql = "SELECT question1_text, question2_text, question3_text, question1, question2, question3 
                FROM activity_progress 
                WHERE studentID = ? AND progID = ?";
        
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ss", $studentID, $progID);
        $stmt->execute();
        $result = $stmt->get_result();
        
        if ($result->num_rows === 0) {
            throw new Exception("No activity progress data found");
        }
        ...
        error_log("Error in getActivityQuestions.php: " . $e->getMessage());
        http_response_code(500);
