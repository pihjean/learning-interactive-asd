...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $requiredFields = ['studentID', 'day', 'category', 'difficulty', 'questionResults', 'timeallotment', 'score', 'questionTexts'];
        ...
        // Check if there's a previous attempt for this student, day, category, and difficulty
        $checkStmt = $conn->prepare("SELECT attempts FROM studentprogress 
                                    WHERE studentID = ? AND day = ? AND category = ? AND difficulty = ?
                                    ORDER BY datetime DESC LIMIT 1");
        if (!$checkStmt) {
            throw new Exception("Prepare failed for check query: " . $conn->error);
        }
        
        $checkStmt->bind_param("ssss", $data['studentID'], $data['day'], $data['category'], $data['difficulty']);
        ...
        $insertStmt = $conn->prepare("INSERT INTO studentprogress 
                                    (studentID, name, day, category, difficulty, 
        ...
        $insertStmt->bind_param(
            "ssssssssssssii", // Changed the timeallotment parameter from 'i' to 's'
            $data['studentID'],
            $studentName,
            $data['day'],
            $data['category'],
            ...
            $q2Text,
            $q3Text,
            $data['timeallotment'], // Now treated as a string
            $data['score'],
            $attempts
        );
